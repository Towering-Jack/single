...
//*BFS
int BFSTraverse(LGraph Graph, const Vertex V)
{
    int count, level;
    Vertex W, last, tail;
    PtrToAdjVNode N;
...
void SDS(LGraph Graph)
{
    ...
    for (V = 1; V <= Graph->NumV; V++)
    {
        count = BFSTraverse(Graph, V);
        printf("%d: %.2lf%%", V, ((double)(count * 100) / Graph->NumV));
        if (V != Graph->NumV)
            printf("\n");
    }
}
